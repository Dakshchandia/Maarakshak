...
import {
  ...
  ChevronLeft, ChevronRight, Sparkles, Pill, Syringe, Apple, User as UserIcon, Phone
} from 'lucide-react';
...
import { AIHealthInsights } from '@/components/common/AIHealthInsights';
...
// ─── Care Team Card ───────────────────────────────────────────────────────────
function CareTeamCard({ pregnancy }: { pregnancy: NonNullable<ReturnType<typeof usePregData>> }) {
  const { t } = useTranslation();
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            {t('woman.yourCareTeam', 'Your Care Team')}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {ashaWorker ? (
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-gray-900">{ashaWorker.name}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          {pregnancy && <CareTeamCard pregnancy={pregnancy} />}
        </div>
      </div>
