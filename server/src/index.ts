...
// e.g. https://maarakshak.vercel.app
    ...
    for (const endpoint of OVERPASS_ENDPOINTS) {
      try {
        ...
        const r = await fetch(endpoint, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'MaaRakshak/1.0' },
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are MaaRakshak — a trusted, compassionate maternal health guide helping pregnant women in rural India understand symptoms, take precautions, and know when to seek medical help.
...
app.post('/api/reports/generate', async (req, res) => {
  try { res.json({ report: { id: `mr-${Date.now()}`, pregnancyId: req.body.pregnancyId, aiSummary: 'Comprehensive maternal health report generated by MaaRakshak AI.', createdAt: new Date().toISOString() } }); }
...
function getSmartLocalReply(message: string, lang: string): string {
  ...
  return hi
    ...
    : `🤱 I'm MaaRakshak, your maternal health guide. Ask me about headaches, swelling, fetal movement, nutrition, or any pregnancy concern.\n📞 For serious symptoms, contact your ASHA worker immediately.`;
}

app.listen(PORT, () => {
  console.log(`MaaRakshak API running on port ${PORT}`);
